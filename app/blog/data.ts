...
export type BlogPost = { category: string; title: string; excerpt: string; status: string; href?: string; image?: string; imageAlt?: string; aiImage?: boolean };

export const posts: BlogPost[]=[
  ...articles.map(article => ({ category: article.category, title: article.title, excerpt: article.description, status: `입문 가이드 · ${article.date.replaceAll('-', '.')}`, href: `/blog/${article.slug}`, image: article.thumbnail.src, imageAlt: article.thumbnail.alt, aiImage: true })),
