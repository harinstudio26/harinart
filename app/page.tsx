...
export default function Home(){return <>
  ...
  <section className="soft section"><div className="section-head"><div><p className="eyebrow">OUR MOMENTS</p><h2>실제 작품과 창작의 순간</h2></div><Link className="text-link" href="/gallery">갤러리 전체보기 <ArrowRight size={16}/></Link></div><div className="home-photo-gallery">{realMoments.map(item=><Link href="/gallery" key={item.label}><figure><img src={item.src} alt={item.alt}/><span>{item.label}</span></figure></Link>)}</div></section>
