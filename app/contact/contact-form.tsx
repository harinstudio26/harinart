...
export function ContactForm({initialType, initialInterest}:{initialType:string;initialInterest:string}) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage('입력 형식은 확인되었습니다. 현재 온라인 전송 기능은 연결 준비 중이며 작성 내용은 전송되거나 저장되지 않았습니다.');
  }
  ...
  return <>
    <section className="contact-section">
      <div className="contact-intro">
        ...
        <div className="contact-note"><span>02</span><p><b>온라인 접수 기능은 연결 준비 중입니다.</b><br/>현재 입력 내용은 서버나 이메일로 전송되지 않습니다.</p></div>
      </div>
      <form className="contact-form contact-form-card" onSubmit={handleSubmit}>
        <p className="form-banner full"><b>현재 안내</b><span>온라인 전송 기능을 연결 중입니다. 이 폼에서 입력한 내용은 아직 저장되지 않습니다.</span></p>
        ...
        <label className="full">관심 프로그램 또는 작품<input name="interest" defaultValue={initialInterest} placeholder="예: 전통매듭 원데이 클래스 또는 작품 종류"/></label>
        <label>희망 일정<input name="date" type="date"/></label>
        <label>예상 인원<input name="headcount" type="number" min="1" placeholder="예: 10"/></label>
        <label className="full">문의 내용 <b>*</b><textarea name="message" rows={7} required placeholder="궁금한 점과 원하는 방향을 자세히 남겨주세요"/></label>
        <label className="privacy full"><input name="privacy" type="checkbox" required/><span><b>*</b> 개인정보 수집·이용 안내를 확인했습니다.<small>실제 온라인 접수 기능이 연결되기 전에는 입력 정보가 전송되거나 저장되지 않습니다.</small></span></label>
        <button className="button contact-submit full" type="submit">입력 내용 확인 <ArrowRight size={17}/></button>
